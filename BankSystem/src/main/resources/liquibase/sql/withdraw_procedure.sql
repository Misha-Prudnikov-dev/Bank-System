CREATE DEFINER=`root`@`localhost` PROCEDURE `withdraw`(IN amount decimal(10,2),IN accountId int,OUT transactionId int )
BEGIN
...
SELECT id_type_transactions into transaction_type_id FROM type_transactions ty_tr WHERE ty_tr.name_type_transactions='СНЯТИЕ';
SELECT balance_accounts into balance FROM accounts acc WHERE acc.id_accounts=accountId;
